$(document).ready(function(e){
    /* 處理文章 */
        /* 文章修改 */
        $('.post__modify > [action="modify_post.php"]').click(function(e) {
            e.preventDefault();
        ...
        $(document).on("submit",'.modify > [action="modify_post.php"]', function(e) {
            e.preventDefault();
            const postId = $(e.target.parentNode).find("input[name=post_id]").val()
                ...
                success: function(resp){
                    ...
                    if (response.result==='success') {
                        ...
                        $(e.currentTarget.parentNode).replaceWith(`
                            <p class="post__read--content card-text">
                                ${modifyContent}
                            </p>
                        `)
                    }
        ...
        /* 文章刪除 */
        $('.post__modify > [action="del_post.php"]').submit(function(e) {
            e.preventDefault();
    ...
    /* 處理迴響 */
        ...
        /* 迴響修改 */
        $(document).on("click",'.comment__modify > [action="modify_comment.php"]', function(e) {
            e.preventDefault();
            /* 取得迴響目前的內容 */
        ...
        /* 停止表單送出 */
        $(document).on("submit",'.modify > [action="modify_comment.php"]', function(e) {
            e.preventDefault()
            const commentId = $(e.target.parentNode).find("input[name=comment_id]").val()
            const modifyContent = $(e.target.parentNode).find("textarea[name=modify_content]").val()
            $.ajax({
        ...
        /* 刪除迴響 ok */
        $(document).on("submit",'.comment__modify > [action="del_comment.php"]', function(e) {
            e.preventDefault();
